...
C_blank = 1
# ここでExcelのデータを取得
shift = pd.read_excel("8月作成用.xlsx", sheet_name="Shift", index_col=0)
manage = pd.read_excel("8月作成用.xlsx", sheet_name="Manage", index_col=0)
member = pd.read_excel("8月作成用.xlsx", sheet_name="Member", index_col=0)
member = member.T
detail = pd.read_excel("8月作成用.xlsx", sheet_name="Detail", index_col=0)
# Shiftシートのカラムとインデックス
date = shift.index
...
# 変数作成
var = pd.DataFrame(
    np.array(addbinvars(days, number_employee)), columns=employee, index=date
)
# ０と１を逆転させている
...
for i in list(range(number_employee)):
    for n, p in enumerate(
        (var.values[:-2, i] + var.values[1:-1, i] + var.values[2:, i]).flat
    ):
...
amount_normal_setting = {
    "max": detail.at["max", "normal"],
    "min": detail.at["min", "normal"],
}
...
for (_, h), (name, n) in zip(
    shift[amount_lock_user_list].iteritems(), var[amount_lock_user_list].iteritems()
):
...
# 目的関数決定
k += (
    C_need_dif * lpSum(shift.V_need_dif)
...
evaluation = pd.DataFrame(
    [amount, count, continuous_work_list],
    index=["シフト希望量", "入る量", "連勤"],
    columns=employee,
)
print(evaluation)
